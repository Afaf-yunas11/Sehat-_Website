...
router.post("/", authorizeUser([userTables.admin], true), async (req, res) => {
  try {
    ...
    if (!(BOOKING_ID && AMOUNT_PAYABLE && MODE_OF_PAYMENT && PAYMENT_STATUS && DISCOUNT)) {
    ...
    const result = await pool
      ...
      .input("AMOUNT_PAYABLE", sql.Decimal(18, 2), AMOUNT_PAYABLE)
...
router.delete("/:id", authenticateToken, authorizeUser([userTables.admin], false), async (req, res) => {
  try {
    ...
    if (result.rowsAffected[0] === 0) {
      return res
        .status(404)
        .json({ error: `TRANSACTION WITH TRANSACTION ID ${id} NOT FOUND` });
    }
    res
      .status(200)
      .json({ message: `TRANSACTION WITH TRANSACTION ID ${id} UPDATED SUCCESSFULLY` });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

router.patch("/by-user/:id", authenticateToken, authorizeUser([userTables.admin], false), async (req, res) => {
  const id = parseInt(req.params.id);
  ...
  for (let field in updates) {
    ...
    if (field === "DISCOUNT" || field === "AMOUNT_PAYABLE") {
